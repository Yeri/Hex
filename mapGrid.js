//  MapGrid
//
// copyright (c) 2009-2013 Mark Butler
// This program is free software; you can redistribute it 
// and/or modify it under the terms of the GNU General Public License
// as published by the Free Software Foundation;
// either version 2 of the License, or (at your option) any later version. 
// reference: http://www.webwargaming.org/mapgrid.js
...
MapData.prototype.setData = function( originX, originY, topHeight, bottomHeight, 
...
function MapGrid(mapData) {
	...
	var distanceFromLeftEdgeOfHexpartRectangle;
	var distanceFromRightEdgeOfHexpartRectangle;
	var distanceFromTopEdgeOfHexpartRectangle;
	var column, row;
...
MapGrid.prototype.calculateHexpartFromPixels = function(pixelX, pixelY) {
  ...
  this.distanceFromLeftEdgeOfHexpartRectangle = (this.mapGridX - this.leftMapEdge) - 
																								(this.column * this.hexagonWidth);

  if (this.distanceFromLeftEdgeOfHexpartRectangle < this.hexsideWidth) {//  it's a / or \ hexside
    hexpartX = (2 * this.column) - 1;
    this.row = Math.floor(this.mapGridY / this.halfHexagonHeight);
    hexpartY = (2 * this.row) + 1;
    this.distanceFromTopEdgeOfHexpartRectangle = this.mapGridY - (this.row * this.topHeight);
  } else { // it's a center or lower hexside
...
, hexpartType;
  ...
  switch (hexpartType) {
    ...
    case 2:
      if (this.distanceFromTopEdgeOfHexpartRectangle < this.oneFourthHexagonHeight) {
    ...
    case 3:
      ...
      // the mappoint tangent is opposite / adjacent =  			
			// this.distanceFromLeftEdgeOfHexpartRectangle / this.distanceFromTopEdgeOfHexpartRectangle
      //
      // is mappoint tangent <  line tangent ?
      // (this.distanceFromLeftEdgeOfHexpartRectangle / 
      // this.distanceFromTopEdgeOfHexpartRectangle) < (this.hexsideWidth / this.topHeight)
      //
      // multiply both sides by this.topHeight
      // (this.distanceFromLeftEdgeOfHexpartRectangle / 
			// this.distanceFromTopEdgeOfHexpartRectangle) * this.topHeight  < (this.hexsideWidth )
      //
      // multiply both sides by this.distanceFromTopEdgeOfHexpartRectangle
      // (this.distanceFromLeftEdgeOfHexpartRectangle * this.topHeight ) < 
			// (this.distanceFromTopEdgeOfHexpartRectangle * this.hexsideWidth)
      //
        
      if (this.distanceFromLeftEdgeOfHexpartRectangle * this.topHeight < 				
					this.distanceFromTopEdgeOfHexpartRectangle * this.hexsideWidth) {
      //  ______
      //  |\ |  |
      //  | \|  |
    ...
    case 4:
      ...
      //   check from right side now
			this.distanceFromRightEdgeOfHexagonRectangle = this.hexsideWidth - 		
				this.distanceFromLeftEdgeOfHexpartRectangle;
      if (this.distanceFromRightEdgeOfHexagonRectangle * this.topHeight < 
					this.distanceFromTopEdgeOfHexpartRectangle * this.hexsideWidth) {
      //  ______
      //  |  | /|
      //  |  |/ |
